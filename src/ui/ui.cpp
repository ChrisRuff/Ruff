...
void Engine::displayImage(const imgproc::Image& img,
                          const uint16_t x,
                          const uint16_t y,
                          const double rotation)
{

    imgproc::Image image = img;
    imgproc::ImageManipulator rotator(image);
	if(rotation != 0) { image = rotator.Rotate(rotation); }

	const auto width = getWidth();
	const auto height = getHeight();
	for(uint16_t i = x; i < image.Width() + x && i < width; ++i)
	{
		for(uint16_t j = y; j < image.Height() + y && j < height;
		    ++j)
		{
			const imgproc::Pixel new_p = image.Get(i - x, j - y);
			draw(i, j, new_p);
		}
	}
}
...
void Engine::draw(const uint16_t x, const uint16_t y, imgproc::Pixel color)
{
	screen->draw(x, y, color);
}
void Engine::draw(const Point2D<uint16_t>& p, const imgproc::Pixel& color)
{
	draw(p.x, p.y, color);
}
                                               ...
                                               const int line_width)
{
	...
	{
		if(x2 < x1) { return getLine(x2, y2, y1, y1, line_width); }
		else
		{
                        ...
                        const bool fill)
{
	...
	else
	{
		for(int x = centerX - radius; x <= centerX + radius; ++x)
		{
			...
			drawLine(x, lowerbound < 0 ? 0 : lowerbound, x, upperbound, color);
		}
	}
}
...
imgproc::Pixel Engine::getPixel(uint16_t x, uint16_t y) const
{
	return screen->get(x, y);
}
imgproc::Pixel Engine::getPixel(Point2D<uint16_t> p) const
{
	return getPixel(p.x, p.y);
}
std::vector<imgproc::Pixel> Engine::getRegion(uint16_t x1,
                                     uint16_t y1,
                                     uint16_t x2,
                                     uint16_t y2)
{
	...
	std::vector<imgproc::Pixel> region{};
	for(uint16_t y = y1; y <= y2; ++y)
	{
		for(uint16_t x = x1; x <= x2; ++x)
		{
			...
			{
				region.push_back(imgproc::BLANK);
			}
			else
			{
...
std::vector<imgproc::Pixel> Engine::getRegion(Point2D<uint16_t> p1,
                                     Point2D<uint16_t> p2)
{
