...
{
	void
	  Image::set(const uint16_t x, const uint16_t y, const Pixel& color)
	{
		if(x >= w || y >= h) { return; }
		pixels[y * w + x] = color;
	}
	...
	void Image::line(const Point2D<uint16_t>& p1,
	...
	void Image::rectangle(const Point2D<uint16_t>& p1,
	                      const Point2D<uint16_t>& p2,
	                      const Pixel& color)
	{
		...
		{
			for(uint16_t j = min_y; j < max_y; ++j) { set(i, j, color); }
		}
	}
	void Image::resize(uint16_t new_w,
	                   uint16_t new_h,
	...
	Image Image::rotate(const double theta,
	                    ROTATION_METHOD rotation_method) const
	{
		...
		const Point2D<uint16_t> new_dims(
		  std::round(std::abs(h * std::cos(theta))
		             + std::abs(w * std::sin(theta)))
		    + 1,
		  std::round(std::abs(w * std::cos(theta))
		             + std::abs(h * std::sin(theta)))
		    + 1);
		...
		for(uint16_t i = 0; i < w; ++i)
		{
			for(uint16_t j = 0; j < h; ++j)
			{
				...
				if(0 <= new_x && new_x < new_dims.x && 0 <= new_y
				   && new_y < new_dims.y)
				{
