...
namespace ui
{
	...
	struct MouseState
	{
		std::array<bool, 3> mouse_pressed{ false, false };
		std::array<bool, 3> mouse_held{ false, false };
		std::array<bool, 3> mouse_released{ false, false };
	...
	public:
		...
		void render(imgproc::Image* image)
		{
		...
		double getDeltaTime()
		{
			...
			double delta =
			  static_cast<double>(now - this->last)
