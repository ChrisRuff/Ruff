...
public:
	Window(const uint16_t width, const uint16_t height, const Pixel& bg) : 
		frame(width, height, bg), bg(bg), 
		renderer(std::make_unique<SDL2_Renderer>(Point2D<uint16_t>{width, height}, title))
	{
	...
	Pixel get(uint16_t x, uint16_t y)
	{
