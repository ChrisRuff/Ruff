...
public:
	[[nodiscard]] constexpr Point(
	  std::array<T, size> vals = {}) noexcept
	  : vals(vals)
	{
	}

	inline constexpr auto
	  operator<=>(const Point<T, size>& other) const noexcept
	{
		// Compare based on distance from
		// origin
		double length1 = 0;
		double length2 = 0;
		std::for_each(vals.begin(), vals.end(), [length1](const T& val) {
			length1 += std::pow(val, 2);
		});
	  ...
	  operator==(const Point<T, size>& other) const noexcept
	{
		for(size_t i = 0; i < size; ++i)
		{
			if(vals[i] != other[i]) { return false; }
		}
		return true;
	}
	...
	inline constexpr Point&
	  operator+=(const Point<T, size>& other) noexcept
	{
		for(size_t i = 0; i < size; ++i) { vals[i] += other[i]; }
		return *this;
	}
	inline constexpr Point
	  operator+(const Point<T, size>& other) const noexcept
	...
	{
		for(size_t i = 0; i < size; ++i) { vals[i] *= -1; }
		return *this;
	};
	inline constexpr Point operator-(const T other) const noexcept
	{
	...
	inline std::string toString() const
	{
		...
		std::for_each(vals.begin(), vals.end() - 1, [&](const T& val) {
			point_text += std::to_string(val) + ", ";
		});
...
public:
	...
	[[nodiscard]] constexpr Point2D(
	  const std::array<T, 2>& vals) noexcept
	  : Point<T, 2>({ vals[0], vals[1] }), x(this->vals[0]),
	    y(this->vals[1])
	{
	}
	~Point2D() = default;

	constexpr Point2D(const Point2D& new_point) noexcept
	  : Point2D(new_point.vals)
	{
	}
	constexpr Point2D(Point2D&& new_point) noexcept
	  : Point2D(new_point.vals)
	{
	}
...
public:
	[[nodiscard]] constexpr Point3D() noexcept
	  : Point<T, 3>(), x(this->vals[0]), y(this->vals[1]),
	    z(this->vals[2])
	{
	}
	[[nodiscard]] constexpr Point3D(const Point<T, 3>& p) noexcept
	  : Point<T, 3>(p), x(this->vals[0]), y(this->vals[1]),
	    z(this->vals[2])
	{
	}
	[[nodiscard]] constexpr Point3D(const T x, const T y) noexcept
	  : Point<T, 3>({ x, y }), x(this->vals[0]), y(this->vals[1]),
	    z(this->vals[2])
	{
	}
	[[nodiscard]] constexpr Point3D(
	  const std::array<T, 3>& vals) noexcept
	  : Point<T, 3>({ vals[0], vals[1] }), x(this->vals[0]),
	    y(this->vals[1]), z(this->vals[2])
	{
	}
	~Point3D() = default;

	constexpr Point3D(const Point3D& new_point) noexcept
	  : Point3D(new_point.vals)
	{
	}
	constexpr Point3D(Point3D&& new_point) noexcept
	  : Point3D(new_point.vals)
	{
	}
