...
namespace fio
{
	template<typename T, typename TP = T::value_type>
	requires std::convertible_to<TP, std::string>
	bool write(const T data_start,
	           const T data_end,
	  ...
	  read(const std::filesystem::path file, const char delimeter = ',')
	{
		...
		if(!in_file.is_open()) { return std::optional<std::vector<T>>(); }
		std::string line{};
