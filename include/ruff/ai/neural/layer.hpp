...
namespace ruff::ai
{
    ...
    public:
        Layer(int neurons_count, std::unique_ptr<ActivationFunction> activation_function) :
            ...
            m_bias(1, neurons_count),
            m_last_output(0,0),
						m_errors(neurons_count, 1),
        ...
        Tensor<FLT_TYPE> Compute(const Tensor<FLT_TYPE>& input)
        {
            ...
            return m_func->ComputeTensor(m_last_output);
        };
        ...
        void SetInputSize(size_t size)  { m_weights.Resize(m_weights.rows(), size); };
