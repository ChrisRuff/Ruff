...
struct Integer
{
	...
	operator std::string() const { return std::to_string(val); }
...
TEST_CASE("Read-Write to file")
{
	...
	// Requires convertable to string
	// template
	ruff::fio::write<std::vector<Integer>::iterator>(
	  vals.begin(), vals.end(), "/tmp/ruff");
